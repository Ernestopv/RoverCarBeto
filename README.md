...
# The camera is mounted rotated 180 degrees, so BOTH flips are required.

# Using only --vflip fixes top/bottom but leaves the image mirrored left/right.

CAMERA_VFLIP = env_flag("CAMERA_VFLIP", True)
CAMERA_HFLIP = env_flag("CAMERA_HFLIP", True)
...
BASE_DIR = Path(**file**).resolve().parent / "static"
...
def start_camera():
    ...
    if CAMERA_SOURCE == "simulator":
        ...
        logging.info(
            ...
            "Starting simulated camera with FFmpeg: %sx%s @ %s FPS, vflip=%s, hflip=%s",
        ...
        # Important: vflip + hflip together equal a 180-degree rotation.
        # Keeping only vflip would make left/right appear mirrored.
        if CAMERA_VFLIP:
        ...
        logging.info(
            ...
            "Starting IMX500 camera: model=%s, postprocess=%s, vflip=%s, hflip=%s",
...
def main():
    ...
    print(
        ...
        f"vflip={CAMERA_VFLIP}, "

        f"hflip={CAMERA_HFLIP}, "
