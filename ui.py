...
# CAMERA ORIENTATION
#
# The simulated FFmpeg camera does not need any physical mounting correction,
# therefore its safe default is 0 degrees.
#
# The real IMX500 on this rover is mounted upside down, therefore its default
# is 180 degrees. An explicit CAMERA_ROTATION environment variable always wins.
#
#   CAMERA_SOURCE=simulator -> default 0
#   CAMERA_SOURCE=imx500    -> default 180
#   CAMERA_ROTATION=0/180   -> explicit override
rotation_raw = os.getenv("CAMERA_ROTATION")

if rotation_raw is None or not rotation_raw.strip():
    CAMERA_ROTATION = 0 if CAMERA_SOURCE == "simulator" else 180
    CAMERA_ROTATION_SOURCE = f"default for {CAMERA_SOURCE}"
else:
    try:
        CAMERA_ROTATION = int(rotation_raw.strip())
    except ValueError:
        print("\nERROR: CAMERA_ROTATION must be 0 or 180\n")
        sys.exit(1)
    CAMERA_ROTATION_SOURCE = "environment"

if CAMERA_ROTATION not in (0, 180):
    print(
        f"\nERROR: Invalid CAMERA_ROTATION: {CAMERA_ROTATION}\n"
        "Available values: 0, 180\n"
    )
    sys.exit(1)
...
def start_camera():
    ...
    if CAMERA_SOURCE == "simulator":
        ...
        if CAMERA_ROTATION == 180:
            # A vertical + horizontal flip is a true 180-degree rotation.
        ...
        logging.info(
            ...
            "Starting simulated camera with FFmpeg: %sx%s @ %s FPS, rotation=%s",
        ...
        if CAMERA_ROTATION == 180:
            # rpicam's two flips together rotate the frame 180 degrees.
            # Do not use only one of them: that would mirror one axis.
            command.extend(["--vflip", "--hflip"])

        command.extend(["--output", "-"])

        logging.info(

            "Starting IMX500 camera: model=%s, postprocess=%s, rotation=%s",
...
def main():
    ...
    print(
        ...
        f"rotation={CAMERA_ROTATION} ({CAMERA_ROTATION_SOURCE}), "
